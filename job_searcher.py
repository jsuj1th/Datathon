...
class JobSearcher:
    ...
    async def search_jobs(self, keywords: str, location: str = "", limit: int = 25) -> List[Dict[str, Any]]:
        ...
        # Try The Muse API first, then fallback to realistic generation
        sources = [
            self._search_themusejobs,
            self._generate_realistic_jobs  # Fallback with realistic data
        ]
        
        for source_func in sources:
            try:
                jobs = await source_func(keywords, location, limit)
                if jobs:
                    all_jobs.extend(jobs)
                    logger.info(f"Got {len(jobs)} jobs from {source_func.__name__}")
    ...
    async def _search_themusejobs(self, keywords: str, location: str, limit: int) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            params = {
                "category": "Software Engineering" if "engineer" in keywords.lower() else "Data Science",
                "page": 1,
                "descending": "true"
            }
            ...
            if response.status_code == 200:
                ...
                for job in data.get("results", [])[:limit]:
                    ...
                    formatted_job = {
                        ...
                        "apply_link": f"https://www.themuse.com/jobs/{job.get('id')}",
                        "location": job_location,
                        "salary": None,  # The Muse API doesn't typically include salary
                        "description": job.get("contents", "")[:200] + "..." if job.get("contents") else None,
                        ...
                        "posted_date": None,  # The Muse API doesn't include posting dates
                        "deadline": None,
