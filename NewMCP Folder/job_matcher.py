...
class JobMatcher:
    ...
    def extract_resume(self, pdf_path: str) -> str:
        """Alias for extract_text_from_pdf - for MCP compatibility"""
        return self.extract_text_from_pdf(pdf_path)

    def match_job_with_resume(self, job: Dict[str, Any], resume_text: str) -> Dict[str, Any]:
        """Match a single job with resume using LLM"""
        try:
            if not self.model:
                # Fallback to keyword matching
                return self._fallback_single_match(job, resume_text)

            prompt = f"""You are an expert job matcher. Analyze this job posting against the resume.
...
Provide a match score (0-100) and brief reason (one sentence).

Respond in JSON format ONLY:
{{
  "score": 85,
  "reason": "Strong Python and ML skills match"
}}
"""
            ...
            matched_job['match_score'] = result.get('score', 0)
            matched_job['match_reason'] = result.get('reason', '')
    ...
    def _fallback_single_match(self, job: Dict[str, Any], resume_text: str) -> Dict[str, Any]:
        """Simple keyword-based matching for a single job"""
        ...
        matched_job = job.copy()
        matched_job['match_score'] = min(score, 100)
        matched_job['match_reason'] = 'Keyword-based match'
...
def main():
    ...
    resume_pdf = str(project_dir / "MCP_Servers" / "pdfDocs" / "Resume_NEW_ML_Pathakota_Pranavi_2.pdf")
